...
test('catch-all renders CMS page at /oferta', async ({ page }) => {
...
test('path routing supports nested page /oferta/pakiety', async ({ page }) => {
  const response = await page.goto('/oferta/pakiety', { waitUntil: 'networkidle' })
  expect(response?.ok()).toBeTruthy()
  await expect(page.locator('body')).toContainText(/pakiety/i)
})

test('preview endpoint rejects missing and invalid secrets', async ({ request }) => {
  const missingSecretResponse = await request.get('/api/preview?path=home')
  expect(missingSecretResponse.status()).toBe(400)

  const invalidSecretResponse = await request.get('/api/preview?path=home&secret=invalid-secret')
...
test('preview endpoint accepts valid secret', async ({ request }) => {
  ...
  const response = await request.get(`/api/preview?path=home&secret=${encodeURIComponent(secret as string)}`)
